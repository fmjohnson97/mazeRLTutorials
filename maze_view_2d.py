...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None,
        ...
        # Set the Goal
        #make random x,y coord for goal
        if gx is None:
            gx=random.randint(1,self.maze_size[0])

        if gy is None:
            gy=random.randint(1,self.maze_size[1])

        self.__goal = np.array(self.maze_size) - np.array((gx, gy))
    ...
    def __draw_entrance(self, colour=(0, 0, 150), transparency=100):

        self.__colour_cell(self.entrance, colour=colour, transparency=transparency)

    def __draw_goal(self, colour=(150, 0, 0), transparency=100):
...
if __name__ == "__main__":
    ...
    maze=MazeView2D(screen_size=(400,400), maze_file_path='/Users/godsrockgirl777/Desktop/Secret Secret/FeudalTests/gym-maze-master/gym_maze/envs/maze_samples/maze2d_003.npy')
    maze.update()
